...
function encrypt_decrypt($action, $string) 
    ...
    function get_pages()
    {
        return $page_arr = array('/home'=>'Home','/about'=>'About Us','/contact'=>'Contact Us','/blog'=>'Blog','/services'=>'Services','/privacy-policy'=>'Privacy Policy','/disclaimer'=>'Disclaimer','/terms-conditions'=>'Terms & Conditions');
    }
...
{
    $file_type=get_file_extension($path,$image);
    if($file_type=='svg'){
        $type='';
    }
...
function get_age_status($status)
{
    if ($status == '1') {
        return '<span class="miniLbl green">Yes</span>';
    } else {
...
function format_amount($amount, $size = 0)
{
    if ($amount!=0) {
...
function get_file_extension($path,$file){
    return pathinfo($path . $file, PATHINFO_EXTENSION);
}
