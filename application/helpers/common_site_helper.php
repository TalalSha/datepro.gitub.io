...
function nextOrder($table,$where='') {
    $CI = get_instance();    
    $CI->db->select("max(order_no) as orderid");
    $CI->db->from($table);
    if(!empty($where)):
        $CI->db->where($where);
    endif;
    $ary= $CI->db->get()->result_array();
    // pr($ary);
...
function saveservices($path, $files, $fieldname, $s_id, $pics, $cont,$thumb_size='') {
    //pr($cont);
    extract($cont);
    $CI = get_instance();
    $cpt = count($files['name']);
...
function upload_product_images($path, $files, $fieldname, $p_id) {
    // pr($files[$fieldname]);
    $success = 0;
    $CI = & get_instance();
    ...
    for ($i = 0; $i < $cpt; $i++) {
        ...
        if ($CI->upload->do_upload($fieldname)) {
            $image = $CI->upload->data();
            savePicture($p_id, $image['file_name']);
            $success++;
        }
    }
    return $success;
...
function get_category_name($id,$type='blog') {
    global $CI;
    $CI = get_instance();
    $CI->db->where('id',$id);
    $CI->db->where('type',$type);
